...
# Utiliser Cloud-Init pour configurer l'IP et le nom d'hôte unique
  ...
  ipconfig0 = var.interfaces_clusters_ip0_node1  # Remplacez par votre passerelle réseau
  ipconfig1 = var.interfaces_clusters_ip1_node1 
  searchdomain = var.dns_name
  nameserver = var.dns_ip


depends_on = [proxmox_vm_qemu.master]
